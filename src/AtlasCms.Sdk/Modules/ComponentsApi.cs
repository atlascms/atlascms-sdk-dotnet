...
/// </summary>
internal sealed class ComponentsApi(AtlasHttpClient http, string restBaseUrl) : IComponentsApi
{
    /// <inheritdoc />
    public Task<PagedResult<Component>> ListAsync(
        ...
        => http.RequestAsync<PagedResult<Component>>(
            Get(Url("/components", query), options), ct);
    ...
    public Task<Component> GetByIdAsync(
        ...
        => http.RequestAsync<Component>(
            Get(Url($"/components/{Enc(id)}"), options), ct);
        ...
        CreateComponentInput payload, AtlasRequestOptions? options = null, CancellationToken ct = default)
    {
        var result = await http.RequestAsync<ApiKeyResult>(
            Post(Url("/components"), payload, options), ct);
    ...
    public Task UpdateAsync(
        ...
        => http.RequestVoidAsync(
            new HttpRequestConfig { Url = Url($"/components/{Enc(payload.Id)}"), Method = HttpMethod.Put, Body = payload, ApiKey = options?.ApiKey }, ct);

    /// <inheritdoc />
    public Task RemoveAsync(
        string id, AtlasRequestOptions? options = null, CancellationToken ct = default)
        => http.RequestVoidAsync(
            new HttpRequestConfig { Url = Url($"/components/{Enc(id)}"), Method = HttpMethod.Delete, ApiKey = options?.ApiKey }, ct);
